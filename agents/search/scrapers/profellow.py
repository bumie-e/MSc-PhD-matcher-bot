"""ProFellow.com scraper. Broad international fellowship database.
...
SELECTORS = {
    "result_card": "article.post, div.fellowship-result",
...
def search(keywords: list[str]) -> list[RawListing]:
    ...
    with httpx.Client(headers=HEADERS, timeout=20.0, follow_redirects=True) as client:
        for keyword in keywords:
            resp = client.get(BASE_URL, params={"s": keyword})
            ...
            for card in soup.select(SELECTORS["result_card"]):
                ...
                href = link["href"]
                source_url = href if href.startswith("http") else f"https://www.profellow.com{href}"
